...
Verified against code on 2026-07-14. Source: component/aitool/, component/ai/.
...
| `delete_template` | `templateId`, `confirmed`, optional `impactToken` | `confirmed=false` previews the exact template and per-device blockers without writing. A later explicitly confirmed call must return the exact preview `impactToken`; stale or blocked commits return `409` with stable `errorCode` and `currentPreview`. Completed deletion refreshes `template_list`. |
...
| `delete_simulation_trace` | Preview a saved run, then delete it only after explicit confirmation in a later user turn (`confirmed=false` then `true`). |
...
| `delete_trace` | Preview a saved verification trace, then delete it only after explicit confirmation in a later user turn (`confirmed=false` then `true`). |
