...
Verified against code on 2026-07-23. Source: component/aitool/, component/ai/.
...
Every successful tool response also reports `objectiveStatus` and `objectiveIssues` from
the final canonical scene. `COMPLETE` means that devices, at least one automation rule,
and at least one formal specification are present; otherwise it is `PARTIAL` with stable
`NO_DEVICES`, `NO_AUTOMATION_RULES`, and/or `NO_SPECIFICATIONS` issue codes. This is a
deterministic minimum-content boundary, not a claim that the natural-language request was
satisfied or that verification passed.
