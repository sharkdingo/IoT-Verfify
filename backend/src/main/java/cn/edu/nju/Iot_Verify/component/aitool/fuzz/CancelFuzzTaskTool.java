...
@Slf4j
@Component
public class CancelFuzzTaskTool extends AbstractAiTool {

    private final FuzzService fuzzService;

    public CancelFuzzTaskTool(FuzzService fuzzService, ObjectMapper objectMapper) {
        super(objectMapper);
    ...
    public LlmToolSpec getDefinition() {
        FunctionParameterSchema schema = new FunctionParameterSchema(
                ...
                Map.of("taskId", Map.of("type", "integer",
                        "description", "Counterexample-search task ID.")),
                List.of("taskId"));
        return LlmToolSpec.of(getName(), "Cancel an async counterexample-search task by taskId.", schema);
    }
    ...
    protected String doExecute(Long userId, String argsJson) {
        try {
            ...
            TaskCancellationResultDto result = fuzzService.cancelTask(userId, taskId);
            ...
            ), "Counterexample-search task cancellation completed.");
        } catch (ArgValidationException e) {
            return e.getErrorResponse();
        } catch (ServiceUnavailableException e) {
            log.warn("cancel_fuzz_task busy: {}", e.getMessage());
            return errorJson(e.getMessage(), "SERVICE_UNAVAILABLE", 503);
        } catch (BaseException e) {
            log.warn("cancel_fuzz_task business error [{}]: {}", e.getCode(), e.getMessage());
            return errorJson(e.getMessage(), "BUSINESS_ERROR", e.getCode());
        } catch (Exception e) {
            log.error("cancel_fuzz_task failed", e);
            return errorJson("Failed to cancel counterexample-search task.", "INTERNAL_ERROR", 500);
        }
    }
    ...
    private static String cancellationMessage(TaskCancellationResultDto result) {
        return switch (result.getCancellationOutcome()) {
            case ACCEPTED -> result.isExecutionMayStillBeStopping()
                    ? "The task was marked CANCELLED; the running search may still be stopping."
                    : "The pending task was marked CANCELLED before execution.";
            case ALREADY_CANCELLED -> "The task was already marked CANCELLED.";
            case ALREADY_COMPLETED -> "The task completed before cancellation; inspect its run for findings.";
