    ...
    public LlmToolSpec getDefinition() {
        ...
        props.put("manifest", Map.of(
                ...
                "description", "Full device manifest JSON defining the device behavior. " +
                        ...
                        "Optional: Icon (UI-only self-contained data:image), InternalVariables, ImpactedVariables, Contents. " +
                        ...
                        "Every ImpactedVariables name must have its domain in this same manifest as an IsInside=false InternalVariable. Set Reads=false on that declaration when the device only affects the value without observing it, so its rules and specifications cannot use it as a condition source; omit Reads (or set true) when the device also reads it. " +
